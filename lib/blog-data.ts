...
export const BLOG_POSTS: BlogPost[] = [
  {
    ...
    readTime: 8,
    content: `
O ENEM 2024 foi aplicado em novembro e reuniu milhões de candidatos em todo o Brasil. Neste artigo, você encontra o gabarito completo com análise das questões mais cobradas de cada área.
...
- **Física:** eletromagnetismo, termodinâmica e ótica
...
As questões de maior dificuldade foram nas áreas de Matemática (geometria espacial e probabilidade) e Física (eletromagnetismo). O TRI penaliza erros em questões fáceis mais do que erros em questões difíceis.
  ...
  },
  {
    ...
    readTime: 7,
    content: `
O ENEM 2023 surpreendeu com questões de alto nível de contextualização. Veja o que caiu em cada área e como se preparar para o próximo ENEM.

## Matemática ENEM 2023 — O que caiu?
...
A média geral ficou próxima de 510 pontos. Notas acima de 650 já são consideradas competitivas para a maioria dos cursos federais pelo SISU.
  ...
  },
  {
    ...
    readTime: 10,
    content: `
A prova de Matemática do ENEM 2023 foi composta por 45 questões de múltipla escolha. Este guia traz os principais temas, a estratégia de resolução e onde praticar.
...
- Função logarítmica: aplicações em pH, decibéis, escala Richter
...
3. **Elimine as alternativas absurdas** — a ordem de grandeza já elimina 2-3 opções
4. **Não passe mais de 4 minutos por questão** — marque e volte depois
...
O ENEM Pro tem todas as questões de Matemática do ENEM 2023 organizadas e categorizadas. Responda uma por uma, veja o gabarito e entenda com explicação da IA.
  ...
  },
  {
    ...
    readTime: 9,
    content: `
A prova de Matemática do ENEM 2024 manteve o nível de contextualização característico do exame. Veja o gabarito, os temas e como se preparar.
...
- Funções do 1º e 2º grau: gráficos e interpretação
- Crescimento exponencial: populações, juros compostos

### Geometria
- Semelhança de triângulos (Teorema de Tales)
- Áreas e perímetros
- Volumes: cilindro, cone e esfera em situações práticas
...
**Dica:** Antes de calcular qualquer coisa, sublinhe o que a questão *pede*, não o que ela dá.
...
Pratique as questões de Matemática do ENEM 2024 e de todos os anos de 2009 a 2024 no ENEM Pro.
  ...
  },
  {
    ...
    readTime: 12,
    content: `
Passar no ENEM não é sobre estudar mais — é sobre estudar o que cai, do jeito que o ENEM pergunta. Este guia é direto ao ponto.

## Por que a maioria falha no ENEM

A maioria dos candidatos comete três erros críticos:

1. **Estudam conteúdo que não cai** — decoram fórmulas que aparecem uma vez em 15 anos
2. **Não praticam com questões reais** — estudam teoria mas não simulam a prova
3. **Não entendem por que erraram** — repetem os mesmos erros
...
Foco na **base**. Priorize os temas de alta frequência:

- **Matemática:** funções, geometria plana e espacial, estatística, probabilidade
- **Ciências da Natureza:** equilíbrio químico, eletricidade, ecologia
- **Ciências Humanas:** história do Brasil (Rep. Velha ao presente), geopolítica
- **Linguagens:** gramática contextualizada, interpretação textual, redação

### 3 meses antes do ENEM

Foco em **simulados**. Responda provas completas por disciplina, com timer. Objetivo: identificar as disciplinas com menor acerto.
...
1. **Introdução:** apresente o tema e tese em 4-5 linhas
2. **Desenvolvimento 1:** argumento + evidência + análise crítica
...
- Entender por que você errou uma questão (não só qual a correta)
- Gerar exemplos alternativos do mesmo conceito
- Resumir conteúdos extensos em pontos-chave
...
|---|---|---|
| Matemática | 550 | 700 |
| Ciências da Natureza | 550 | 680 |
| Ciências Humanas | 580 | 720 |
| Linguagens | 560 | 700 |
| Redação | 700 | 900 |

Atingir 600 em média e 700+ na redação já garante acesso a cursos de alta demanda em universidades estaduais.
...
Todas as 5 áreas têm peso igual na média do SISU. Porém, alguns cursos aplicam pesos específicos (ex: Medicina costuma pesar mais C. Natureza). Verifique o edital do curso desejado.
  ...
  },
  {
    ...
    readTime: 6,
    content: `
Praticar com questões reais é a forma mais eficiente de se preparar para o ENEM. Veja como montar seu simulado gratuito.

## Por que usar questões reais do INEP?

Questões inventadas por cursinhos e professores não replicam o **estilo de raciocínio** do ENEM. O INEP tem um padrão muito específico de contextualização, nível de dificuldade e distribuição de alternativas.
...
Escolha uma disciplina e responda 45 questões do mesmo ano. Calcule sua porcentagem de acerto e compare com a média nacional daquele ano.

**Recomendação:** comece com Matemática 2020 — prova de nível médio, boa referência.

### Opção 2: Mini-simulado de 10 questões (20 min)

Para dias com pouco tempo. Misture disciplinas ou foque em um tópico específico. Ideal para manter a consistência.

### Opção 3: Simulado completo (5h30)
...
|---|---|---|
| Abaixo de 40% | Base fraca | Revisar teoria + resolver questões básicas |
| 40–55% | Em desenvolvimento | Aumentar volume de questões |
...
O ENEM Pro tem todas as questões do ENEM de 2009 a 2024 organizadas por disciplina e ano. 10 questões por dia grátis, sem cartão de crédito.
  ...
  },
  {
    ...
    readTime: 11,
    content: `
A redação do ENEM vale 1.000 pontos — 20% da nota total. Candidatos que dominam a redação têm vantagem enorme no SISU. Aqui está a estrutura que funciona.

## As 5 Competências do INEP

O INEP avalia a redação em 5 competências, cada uma valendo até 200 pontos:

1. **Competência 1 — Domínio da língua escrita**
Gramática, concordância, ortografia, pontuação. Erros graves de gramática derrubam toda a nota, pois afetam a competência 1.

2. **Competência 2 — Compreensão da proposta**
Desenvolvimento do tema proposto, sem tangenciar ou fugir do assunto. Fuga completa ao tema = nota 0.

3. **Competência 3 — Argumentação**
Capacidade de selecionar, organizar e interpretar informações para defender um ponto de vista. Use dados, referências culturais e argumentos lógicos — não opiniões pessoais sem fundamento.

4. **Competência 4 — Coesão textual**
Uso correto de conectivos, pronomes e referências para criar um texto fluido. Evite repetição de palavras e frases desconexas.
...
- Apresente o recorte temático (a perspectiva que você vai defender)
- Termine com a tese (o que você vai argumentar)

**Exemplo:** "Segundo dados do IBGE, X% da população brasileira enfrenta Y. Esse cenário evidencia que [tema] é resultado de [causa 1] e [causa 2], problemas que exigem ação imediata do Estado e da sociedade civil."

### Desenvolvimento 1 (8–10 linhas)
- **Argumento:** apresente sua primeira causa/perspectiva
- **Evidência:** dado, exemplo histórico ou citação que fundamenta o argumento
- **Análise:** conecte a evidência ao problema apresentado na introdução

### Desenvolvimento 2 (8–10 linhas)
- **Argumento:** segunda causa/perspectiva (diferente e complementar ao 1º)
- **Evidência:** fundamentação diferente do primeiro parágrafo
- **Análise:** aprofundamento crítico

### Conclusão (5–7 linhas)
**Fórmula:** Agente + ação + meio + finalidade + efeito
...
- **Saúde mental e pressão social**
- **Sustentabilidade e mudanças climáticas**
- **Desinformação e fake news**
- **Inclusão de grupos minorizados**

Qualquer que seja o tema, a **estrutura é a mesma**. Quem domina a estrutura não depende de "adivinhar" o tema.
...
Interpretar bem os textos motivadores da coletânea é 50% da redação. Treine sua leitura crítica com questões de Linguagens do ENEM Pro.
  ...
  },
  {
    ...
    readTime: 8,
    content: `
Estudar muito não é o mesmo que estudar bem. Estas 10 dicas são baseadas no padrão das provas do ENEM de 2009 a 2024.
...
10 questões por dia são 300 questões por mês — um volume suficiente para melhorar significativamente sua performance. Comece grátis no ENEM Pro.
  ...
  },
  {
    ...
    readTime: 9,
    content: `
Ciências da Natureza é uma das quatro áreas do ENEM. A prova reúne Física, Química e Biologia em 45 questões. Veja o que cai mais e como estudar.

## Distribuição das Questões

A distribuição não é fixa, mas a tendência histórica é:
- **Biologia:** 17–20 questões
- **Química:** 13–16 questões
- **Física:** 10–13 questões
...
**Temas mais frequentes (2009–2024):**
- Eletromagnetismo: circuitos, força magnética, indução
- Termodinâmica: máquinas térmicas, 1ª e 2ª lei
- Ótica: refração, lentes, cores
- Ondulatória: som, frequência, velocidade de propagação
- Mecânica: forças, energia, trabalho
...
- Estequiometria: reagente limitante, pureza, rendimento
- Termoquímica: entalpia, lei de Hess
- Eletroquímica: pilha, eletrólise, oxidação
- Química Orgânica: funções, reações de adição e substituição, polímeros
- Equilíbrio químico: Le Chatelier, Kc, Ka, Kb
...
- Genética: leis de Mendel, codominância, heredograma
- Evolução: teoria sintética, seleção natural, especiação
- Biologia celular: organelas, divisão celular (mitose/meiose)
- Saúde: doenças infecciosas, sistema imune, vacinação
- Biotecnologia: DNA recombinante, transgênicos, PCR
...
O ENEM Pro tem todas as questões de Ciências da Natureza de 2009 a 2024. Responda, veja o gabarito e entenda com IA.
  ...
  },
  {
    ...
    readTime: 8,
    content: `
Ciências Humanas reúne História, Geografia, Filosofia e Sociologia em 45 questões. É uma das áreas com maior potencial de nota para quem estuda os temas certos.

## Distribuição Histórica

- **História:** 18–20 questões
- **Geografia:** 14–16 questões
- **Filosofia:** 5–7 questões
- **Sociologia:** 5–7 questões
...
**Temas de maior frequência:**
- **História do Brasil:** período colonial, império, república (Vargas, Ditadura Militar, Nova República)
- **História Geral:** Primeira e Segunda Guerra Mundial, Guerra Fria, descolonização africana
- **América Latina:** independências, populismos, ditaduras militares
- **Temas contemporâneos:** globalização, neoliberalismo, conflitos do século XXI
...
- **Urbanização:** segregação, metropolização, favelização
...
**Filosofia:** filósofos clássicos (Platão, Aristóteles, Kant, Hegel, Nietzsche) e suas principais ideias em contexto.

**Sociologia:** conceitos de Marx, Durkheim e Weber aplicados a situações sociais contemporâneas.
...
Todas as questões de Ciências Humanas do ENEM 2009 a 2024 estão no ENEM Pro. Responda e entenda o raciocínio com IA.
  ...
  },
  {
    ...
    readTime: 10,
    content: `
Um cronograma de estudos eficiente não é aquele que você consegue seguir 100% do tempo — é aquele que você consegue seguir na maioria do tempo. Realismo é fundamental.

## Princípios do Cronograma

Antes de montar o cronograma, defina:

1. **Quantas horas por dia você consegue estudar de fato** (não o que você *gostaria*)
...
|--------|---------|-------|--------|--------|-------|--------|
| 1–4 | Matemática | C. Natureza | C. Humanas | Linguagens | Redação | Revisão |
| 5–8 | Matemática | C. Natureza | C. Humanas | Linguagens | Redação | Simulado |

**Meta:** 1h30 de estudo por dia + 10 questões do ENEM Pro antes de dormir.

### Meses 3–4: Questões e Identificação de Lacunas
...
Não. O ENEM cobre a Matriz de Referência do INEP, que é um recorte dos temas mais importantes. Em Matemática, por exemplo, os 7 temas principais representam 70–80% da prova. Focar nesses temas é muito mais eficiente do que revisar tudo.
  ...
  },
  {
    ...
    readTime: 7,
    content: `
A nota do ENEM não é uma porcentagem de acertos. O cálculo usa a **Teoria de Resposta ao Item (TRI)** — um sistema que considera dificuldade das questões e padrão de respostas. Entender isso muda a estratégia de estudo.

## O que é a TRI?

A TRI (Teoria de Resposta ao Item) é o modelo estatístico que o INEP usa para calcular as notas do ENEM. A TRI considera:

1. **Dificuldade da questão:** questões mais difíceis valem mais quando acertadas
2. **Padrão de respostas:** acertar uma questão difícil e errar a fácil do mesmo tema gera suspeita de "chute" e pode reduzir a nota
3. **Acertos consistentes:** acertar questões progressivamente mais difíceis do mesmo tema gera nota maior

## Como a Nota é Calculada?

O ENEM gera 5 notas:
- Linguagens e Códigos: 0–1000
- Ciências Humanas: 0–1000
- Ciências da Natureza: 0–1000
- Matemática: 0–1000
- Redação: 0–1000

Cada nota usa uma escala TRI com média em torno de 500 pontos para os candidatos que fizeram a prova regularmente.

## Qual Nota Preciso para o Curso que Quero?

A nota de corte para cada curso varia todo ano, mas existem referências históricas:

| Curso | Nota estimada (média das 5 notas) |
|-------|----------------------------------|
| Medicina — UFMG | 780+ |
| Direito — USP | 720+ |
| Engenharia — UFRJ | 680+ |
| Administração — UFF | 620+ |
| Pedagogia — UFPE | 560+ |
...
O SISU usa a **média aritmética simples** das 5 notas do ENEM:
    ...
    Média = (Linguagens + Humanas + C. Natureza + Matemática + Redação) / 5

Alguns cursos usam **pesos diferentes** para cada disciplina. Medicina, por exemplo, costuma dar peso maior para C. Natureza. Verifique o peso específico de cada curso no edital do SISU.

## Estratégia: Onde Concentrar Esforço?

Se você já tem 650 em quatro disciplinas e 400 na redação, melhorar a redação de 400 para 700 aumenta sua média em 60 pontos — o equivalente a melhorar 60 pontos em cada uma das outras 4 disciplinas.

**Foque na sua maior fraqueza.** O impacto marginal de melhorar a nota mais baixa é sempre maior do que melhorar a nota mais alta.
...
Use a ferramenta "Calcular Nota" do ENEM Pro: insira seus acertos em cada área (estimados via gabarito) e a nota da redação para obter uma estimativa TRI.
  ...
  },
  {
    ...
    readTime: 9,
    content: `
Física costuma ser o "medo" de muitos candidatos no ENEM. Mas o segredo é simples: o ENEM não quer que você calcule — quer que você entenda os conceitos e aplique em contexto.
...
- Circuitos em série e paralelo: resistência equivalente, corrente, tensão
- Lei de Ohm: relação V = R·I em contextos práticos
- Potência elétrica: consumo de energia, conta de luz
- Campo magnético: força sobre cargas e condutores

**Dica:** Questões de eletricidade frequentemente trazem tabelas de componentes elétricos. Aprenda a calcular resistência equivalente em circuitos mistos.

### 2. Termodinâmica (8–10% das questões de Física)

- Leis da termodinâmica (especialmente 1ª e 2ª lei)
- Máquinas térmicas: eficiência, trabalho, calor
- Dilatação: linear, superficial, volumétrica

### 3. Ondulatória e Acústica (8–10%)

- Velocidade, frequência e comprimento de onda
- Efeito Doppler: fonte em movimento
- Reflexão, refração, difração

### 4. Ótica (7–9%)

- Reflexão: lei da reflexão, espelhos planos e curvos
- Refração: lei de Snell, índice de refração
- Lentes: convergentes e divergentes, formação de imagem

### 5. Mecânica — Dinâmica (7–9%)

- Segunda lei de Newton: F = ma em situações variadas
- Trabalho e energia: energia cinética, potencial, conservação
- Impulso e quantidade de movimento

### 6. Mecânica — Cinemática (6–8%)

- Movimento uniforme: velocidade, posição, tempo
- Movimento uniformemente variado: aceleração, queda livre
- Gráficos de posição e velocidade × tempo

### 7. Gravitação (4–6%)

- Lei de gravitação universal
- Satélites: velocidade orbital, período
...
### 10. Física Moderna e Quântica (3–5%)
...
3. **Resolva questões do ENEM** (não de vestibulares regionais)
4. **Quando errar, pergunte "por quê?"** — não apenas qual a resposta correta
...
Resolva questões de Física do ENEM de 2009 a 2024 no ENEM Pro. A IA explica o raciocínio físico de cada questão.
  ...
  },
  {
    ...
    readTime: 7,
    content: `
A pergunta "quanto tempo preciso estudar?" tem uma resposta honesta que nenhum cursinho vai te dar: **depende de onde você está e onde quer chegar.**
...
**Horas por dia:** 2–3 horas
**Meta realista:** 550–620 por área

### Candidato com base média, estudando há 1 ano
**Tempo necessário:** 4–6 meses
**Horas por dia:** 1–2 horas
**Meta realista:** 650–720 por área

### Candidato com base boa, querendo otimizar
**Tempo necessário:** 3–4 meses
**Horas por dia:** 1–1.5 horas focadas
**Meta realista:** 720–780 por área

### Candidato mirando cursos de altíssima concorrência (Medicina USP)
**Tempo necessário:** 12–18 meses
...
O resultado de aprendizado não é o mesmo. A distribuição ao longo do tempo é mais eficiente para reter conteúdo.
...
Não espere ter o cronograma perfeito. Comece com 10 questões hoje. Grátis, sem cartão de crédito.
  ...
  },
  {
    ...
    readTime: 8,
    content: `
Linguagens, Códigos e suas Tecnologias é uma das quatro áreas do ENEM, com 45 questões incluindo Língua Portuguesa, Literatura, Artes, Educação Física e Língua Estrangeira (Inglês ou Espanhol).
...
**Temas mais frequentes:**
- **Coesão e coerência textual:** conectivos, pronomes, progressão temática
- **Intertextualidade:** relação entre textos de diferentes épocas ou gêneros
- **Figuras de linguagem:** metáfora, ironia, hipérbole e seus efeitos no texto
- **Variação linguística:** diferenças regionais, sociais e históricas da língua
...
**Movimentos mais cobrados:**
- **Modernismo brasileiro (1ª fase):** Oswald de Andrade, Mário de Andrade, Manuel Bandeira
- **Modernismo (2ª fase):** Drummond, Cecília Meireles, João Cabral
- **Regionalismo:** Guimarães Rosa, Graciliano Ramos, Jorge Amado
...
**Estratégia:**
1. Leia o título e as questões antes de ler o texto
2. Use cognatos (palavras parecidas com o português)
3. Identifique palavras-chave que se repetem
4. Responda pelo contexto, não pela tradução literal
...
- O que a imagem sugere/critica (conotação)
- A relação entre imagem e texto verbal
...
const EXTRA_POSTS_3: BlogPost[] = [
  {
    ...
    readTime: 5,
    content: `
Medicina Veterinária é um dos cursos mais procurados fora da área médica humana. Com mercado aquecido em agronegócio, pets e saúde pública, as notas de corte nas federais variam bastante por região.

## Nota de Corte para Veterinária — SISU 2025 (Referência)

| Universidade | Turno | Nota de Corte |
|---|---|---|
| USP (São Paulo) | Integral | 700–750 pts |
| UFRGS | Integral | 670–710 pts |
| UFMG | Integral | 660–700 pts |
| UnB | Integral | 650–690 pts |
| UFRJ | Integral | 630–670 pts |
| UFC | Integral | 610–650 pts |
| UFV (Viçosa) | Integral | 620–660 pts |
| UFBA | Integral | 590–630 pts |
...
|---|---|---|
| Nota de corte | 730–810 pts | 540–750 pts |
...
**10 questões por dia grátis** — sem cartão de crédito.
  ...
  },
  {
    ...
    readTime: 7,
    content: `
O FIES (Fundo de Financiamento Estudantil) é o programa do governo federal que financia cursos de graduação em faculdades privadas. Se você não conseguiu vaga no SISU (universidade federal) ou quer estudar numa particular, o FIES pode financiar parte ou toda a sua mensalidade.
...
- Ter participado do ENEM a partir de 2010 com nota **mínima de 450 pontos** na média das provas objetivas
- **Não ter zerado a redação**
- Ter renda familiar bruta mensal per capita de até **3 salários mínimos**
...
**FIES Regular (com juros):**
- Financia entre 50% e 100% da mensalidade
- Taxa de juros: 3,4% ao ano
- Carência durante o curso + 12 meses após a formatura
- Prazo de pagamento: até 3 vezes o tempo do curso

**FIES Social (sem juros para renda mais baixa):**
- Para famílias com renda até 1 salário mínimo per capita
...
3. **Preencha a ficha** com dados pessoais, renda familiar e curso desejado
4. **Informe o número do CPF e nota do ENEM** — o sistema valida automaticamente
5. **Aguarde o resultado** — aprovados recebem contrato de financiamento para assinar

O período de inscrições normalmente ocorre em **fevereiro** (1ª edição) e pode haver uma 2ª chamada mais tarde no ano. Acompanhe o calendário oficial.

## FIES vs ProUni — Qual Escolher?

| Critério | ProUni | FIES |
|---|---|---|
| Tipo | Bolsa (não paga) | Financiamento (paga após formatura) |
| Cobertura | 50% ou 100% | 50% a 100% |
| Renda máxima | 3 s.m. per capita (integral) / 1,5 s.m. (parcial) | 3 s.m. per capita |
| Nota mínima ENEM | 450 pts + redação > 0 | 450 pts + redação > 0 |
| Juros | Nenhum | 3,4% a.a. (ou 0% no FIES Social) |
...
Para cursos como Medicina e Direito em faculdades particulares via FIES, espere exigências de 550–650 pontos. Para cursos como Pedagogia, Administração e Letras, 450–500 pontos costumam ser suficientes.

## Posso Ter FIES e ProUni ao Mesmo Tempo?
...
**10 questões por dia grátis** — sem cartão de crédito.
  ...
  },
  {
    ...
    readTime: 7,
    content: `
O ENEM 2025 foi aplicado em novembro de 2025 e o resultado foi divulgado em janeiro de 2026. Neste artigo você encontra análise do gabarito, dificuldade das provas e notas de corte estimadas para o SISU 2026.
...
| Gabarito preliminar | 10 de novembro de 2025 |
...
- **Funções** — logarítmicas e exponenciais aplicadas a crescimento e decaimento (6 questões)
- **Geometria espacial** — volume de sólidos e áreas de superfície (5 questões)
- **Probabilidade e estatística** — análise de dados e eventos compostos (5 questões)
- **Progressões** — PA e PG em contexto financeiro (4 questões)
- **Trigonometria** — no triângulo e círculo trigonométrico (3 questões)
...
- Ecologia — biomas brasileiros, impacto ambiental (4 questões)
- Fisiologia — sistema imunológico, hormônios (3 questões)

### Química
- Química orgânica — polímeros e reações (5 questões)
- Eletroquímica — pilhas e baterias (3 questões)
- Equilíbrio químico — Princípio de Le Chatelier (3 questões)

### Física
- Eletrodinâmica — lei de Ohm, resistência e potência (4 questões)
...
- **Geopolítica contemporânea** — conflitos, globalização e blocos econômicos
...
| Curso | Estimativa de Corte |
|---|---|
| Medicina (federal capital) | 740–810 pts |
| Medicina (federal interior) | 700–760 pts |
| Direito (federal capital) | 650–720 pts |
| Engenharia (federal) | 610–680 pts |
| Arquitetura (federal) | 640–700 pts |
| Administração (federal) | 580–640 pts |
| Enfermagem (federal) | 560–640 pts |
| Pedagogia (federal) | 500–560 pts |
...
1. **SISU** — use para entrar em universidades federais e estaduais
2. **ProUni** — bolsas em faculdades particulares
...
**10 questões por dia grátis** — sem cartão de crédito.
  ...
  },
  {
    ...
    readTime: 8,
    content: `
O SISU (Sistema de Seleção Unificada) é o programa do MEC que usa a nota do ENEM para distribuir vagas em universidades públicas federais e estaduais. Entender como ele funciona é tão importante quanto estudar — candidatos com a mesma nota podem ter resultados muito diferentes dependendo da estratégia.
...
**O processo:**
1. Você se inscreve nas 2 opções durante o período de inscrições (geralmente 4–5 dias)
2. Pode **alterar suas opções** quantas vezes quiser enquanto as inscrições estão abertas
3. Acompanha a **nota de corte parcial** em tempo real — o sistema atualiza conforme mais candidatos se inscrevem
...
- **Ampla concorrência (AC):** qualquer candidato pode concorrer
- **L1:** escola pública, renda familiar ≤ 1,5 salário mínimo per capita, autodeclaro preto/pardo/indígena (PPI)
- **L2:** escola pública, renda ≤ 1,5 s.m. per capita, qualquer raça
- **L5:** escola pública, renda > 1,5 s.m. per capita, PPI
- **L6:** escola pública, renda > 1,5 s.m. per capita, qualquer raça
- Outras modalidades para PCD (pessoas com deficiência)
...
### Estratégia 1 — Sonho + Seguro
- 1ª opção: seu curso/universidade ideal (mesmo que arriscado)
- 2ª opção: curso/local com nota de corte histórica bem abaixo da sua nota

Vantagem: você pode entrar no curso dos sonhos. Risco: se não passar em nenhuma, fica sem vaga.

### Estratégia 2 — Ambas com chance real
- 1ª e 2ª opções: cursos com nota de corte do ano anterior próxima à sua nota (com margem de 20–30 pts)

Vantagem: maior chance de aprovação em pelo menos uma. Risco: pode não entrar no curso preferido.

### Estratégia 3 — Por localização
- 1ª opção: cidade preferida
- 2ª opção: outra cidade com nota menor para o mesmo curso
...
**Cuidado com o efeito manada:** muitos candidatos trocam de opção no último dia, causando variações bruscas na nota de corte. Não tome decisões impulsivas — avalie a tendência ao longo dos dias.

## Quanto Antes Melhor? Quando Alterar sua Opção?

Você pode mudar suas opções a qualquer momento durante as inscrições. **Não há vantagem em se inscrever no primeiro dia** — o que importa é a nota final, não quando você se inscreveu.

Monitore as notas parciais nos primeiros dias para sentir a tendência, depois decida com mais informação.

## O que Acontece Depois do Resultado?

**Aprovado na 1ª opção:** você é automaticamente eliminado da 2ª opção e deve seguir o processo de matrícula.

**Aprovado na 2ª opção:** segue para matrícula no curso da 2ª opção.

**Não aprovado em nenhuma:** você pode entrar na **lista de espera**. Se candidatos aprovados não matricularem, as vagas são redistribuídas para quem está na lista de espera, por ordem de nota.
