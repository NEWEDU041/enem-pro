...
import { YEARS, DISCIPLINES, disciplineToSlug } from '@/lib/enem-constants'
...
export default function QuestoesClient({
  ...
  async function reload(nextYear: string, nextDiscipline: string, nextPage: number) {
    setLoading(true)
    const p = new URLSearchParams({ year: nextYear, page: nextPage.toString() })
    if (nextDiscipline) p.set('discipline', nextDiscipline)
  ...
  function onYearChange(v: string) {
    setYear(v)
    setPage(1)
    reload(v, discipline, 1)
  }

  function onDisciplineChange(v: string) {
    setDiscipline(v)
    setPage(1)
    reload(year, v, 1)
  }
    ...
    <div className="min-h-screen bg-zinc-50">
      ...
      <main className="max-w-5xl mx-auto px-6 py-10">
        ...
        <div className="bg-white rounded-2xl border border-zinc-200 p-6 mb-8 grid md:grid-cols-2 gap-4">
          <div>
            <label htmlFor="questoes-ano" className="block text-sm font-medium text-zinc-700 mb-1">Ano</label>
            <select
              id="questoes-ano"
              value={year}
          ...
          <div>
            <label htmlFor="questoes-disciplina" className="block text-sm font-medium text-zinc-700 mb-1">Disciplina</label>
            <select
              id="questoes-disciplina"
              value={discipline}
        ...
        {loading ? (
          ...
          <>
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        <span className="text-xs text-zinc-500">ENEM {q.year} — Questão {((page - 1) * 20) + i + 1}</span>
                      </div>
            ...
            <div className="flex justify-center gap-3 mt-8">
              {page > 1 && (
                <button onClick={() => onPageChange(page - 1)} className="px-4 py-2 border border-zinc-300 rounded-lg text-sm hover:bg-zinc-50">
                  ← Anterior
                </button>
              )}
