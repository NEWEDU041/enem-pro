...
console.log(`Processing ${posts.length} extracted posts...`);
...
// Expand single post with templates
function expandPost(post, postNumber) {
  const topic = extractTopic(post.title);
  let newContent = post.content;

  // Add expansions
  newContent += expansionTemplates.deepAnalysis(topic);
  newContent += expansionTemplates.examExamples(topic);
  newContent += expansionTemplates.statistics(topic);
  newContent += expansionTemplates.strategies(topic);
  newContent += expansionTemplates.checklist(topic);
  newContent += expansionTemplates.faq(topic);
  ...
  return {
    ...
    stats: {
      ...
      newWords,
      addedWords,
      originalReadTime: post.readTime,
      newReadTime
    }
  };
}
...
for (let i = 0; i < posts.length; i++) {
  ...
  const parsed = parsePost(postData.text);

  if (parsed) {
    const expanded = expandPost(parsed, i + 1);
    expandedPosts.push(expanded);

    stats.totalWordsAdded += expanded.stats.addedWords;
    stats.totalOriginalWords += expanded.stats.originalWords;
    stats.totalNewWords += expanded.stats.newWords;
    stats.originalReadTimes.push(expanded.stats.originalReadTime);
    stats.newReadTimes.push(expanded.stats.newReadTime);

    if ((i + 1) % 10 === 0) {
      console.log(`Expanded ${i + 1}/${posts.length} posts...`);
    }
  } else {
    console.error(`Failed to parse post ${i + 1}`);
  }
}

// Calculate statistics
const avgWordsAdded = Math.round(stats.totalWordsAdded / stats.totalPosts);
const avgOriginalReadTime = Math.round(
  stats.originalReadTimes.reduce((a, b) => a + b, 0) / stats.originalReadTimes.length
);
const avgNewReadTime = Math.round(
  stats.newReadTimes.reduce((a, b) => a + b, 0) / stats.newReadTimes.length
);
const minReadTime = Math.min(...stats.newReadTimes);
const maxReadTime = Math.max(...stats.newReadTimes);

console.log('\n=== EXPANSION COMPLETE ===');
console.log(`Posts expanded: ${expandedPosts.length}`);
console.log(`Average words added per post: ${avgWordsAdded}`);
console.log(`Average original readTime: ${avgOriginalReadTime} min`);
console.log(`Average new readTime: ${avgNewReadTime} min`);
console.log(`Min readTime: ${minReadTime} min`);
console.log(`Max readTime: ${maxReadTime} min`);
console.log(`Total words added: ${stats.totalWordsAdded}`);
console.log(`Total new words in posts: ${stats.totalNewWords}`);

// Save expanded posts
fs.writeFileSync(
  path.join(__dirname, '../posts-expanded.json'),
  JSON.stringify({
    expandedPosts,
    stats: {
      ...stats,
...
console.log('\nSaved expanded posts to posts-expanded.json');
